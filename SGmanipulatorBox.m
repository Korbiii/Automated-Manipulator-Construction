...
%   increased_power:  1xn matrix, defines which dofs need big motor
%	=== OUTPUT RESULTS ======
%
function [SG] = SGmanipulatorBox(dofs,varargin)
%%Varargin
increased_power=0;  if nargin>=2 && ~isempty(varargin{1}); increased_power=varargin{1}; end
...
box_height = 70;
box_height_2 = SM40BL_dimensions(3);
...
for i=1:dofs
    if ~ismember(i,increased_power)
        box_height_2 = SM85CL_dimensions(3);
        PL_side = PLtrans(CPLbool('-',PLsquare(10,SM40BL_dimensions(1)+5),PLtrans(PLsquare(5,SM40BL_dimensions(1)+5),[2.5 5])),[-SM40BL_dimensions(2)/2 0]);
    else
        PL_side = PLtrans(CPLbool('-',PLsquare(10,SM85CL_dimensions(1)+5),PLtrans(PLsquare(5,SM85CL_dimensions(1)+5),[2.5 5])),[-SM85CL_dimensions(2)/2 0]);
    end
    ...
    distance_to_next_mid = max((rotor_radius+5)-(sizex/2),0)+10+(sizex/2);
    if isempty(SG_servo_guides)
        SG_servo_guides = SGtrans(SG_guide,[0 -sizey/2 0]);
        sizex_first = sizex/2;
    else
        distance_next = max((rotor_radius+5)-(sizex/2),0)+max(rotor_radius-(sizex_old/2),0)+20;
    ...
    if isempty(PL_axle_positions)
        ...
        PL_axle_positions = [PL_axle_positions;distance_to_next_mid_old+distance_to_next_mid-5+PL_axle_positions(end,1),axle_y_dis];
    end
...
SG_main_frame = SGofCPLz(PL_main_frame,box_height_2+2);
...
for k=1:size(PL_axle_positions,1)
    PL_main_frame_top = [PL_main_frame_top;NaN NaN;PLtrans(PLcircle(16),[-PL_axle_positions(k,1)-(box_wall_thickness*2)-sizex_first;-PL_axle_positions(k,2)])];
    if ismember(k,increased_power)
        PL_main_frame_top = [PL_main_frame_top;NaN NaN;PLtrans(PL_sm85_holes,[-PL_axle_positions(k,1)-(box_wall_thickness*2)-sizex_first;-PL_axle_positions(k,2)+16])];
    end
end
...
if sizex+30 > 190
    ...
    SG_close = SGofCPLz(PL_close,box_height_2+12);
...
SG_back_plate = SGtransrelSG(SG_back_plate,SG_main_frame_side,'aligntop',13,'alignright','infront',-10);

SG_bottom_plate = SGofCPLx(PLsquare(17,10),170);
...
for k=1:dofs
    if ~ismember(k,push_rod)
        PL_top_plate = [PL_top_plate;NaN NaN;PLtrans(PLcircle(16),[-PL_axle_positions(k,1)-(2*box_wall_thickness)-sizex_first;-PL_axle_positions(k,2)-box_wall_thickness])];
    else
        PL_top_plate = [PL_top_plate;NaN NaN;PLtrans(PLsquare(18,60),[-PL_axle_positions(k,1)-(2*box_wall_thickness)-sizex_first;-PL_axle_positions(k,2)-box_wall_thickness])];
    end
end
...
SG_connection_top = SGstack('z',SG_connector,SG_connection_top_top,SG_connection_top_through);
...
SG_connection_top_85 = SGstack('z',SG_connector_85,SG_connection_top_top_85,SG_connection_top_through_85);
...
for i=1:dofs
    if ~ismember(i,push_rod)
        ...
        SG_sledge_guide = SGtransrelSG(SG_sledge_guide,SG_top_plate,'ontop',-12.4,'alignfront',-5,'transx',-PL_axle_positions(i,1));
        SG_sledge = SGstack('z',SG_sledge,SG_sledge_slot);
        SG_rotors_top_plate{end+1} = SGtransrelSG(SG_sledge,SG_top_plate,'ontop',-10.2,'transy',-PL_axle_positions(i,2),'transx',-PL_axle_positions(i,1));
