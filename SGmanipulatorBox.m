...
%	=== OUTPUT RESULTS ======
%
function [SG] = SGmanipulatorBox(power,varargin)
%% Variables defining
SGm = []; if nargin>=2 && ~isempty(varargin{1}); SGm=varargin{1}; end
...
%% Calculating ranges etc
if ~isempty(SGm)
    for i=1:SGm{1}.numArms
        tempcount = 2;
        for j=1:SGm{1}.numSections(i)
            tempcount = tempcount + (SGm{1}.numElements{1}(j))+1;
            CPL_temp = SGm{tempcount}.CPL{1};           
